...
class AreaCoordinatorCreateSerializer(serializers.Serializer):
    # ------ MEMBER ------ #
    ...
    organization_type_of = serializers.ChoiceField(
        required=False,
        allow_null=True,
        choices=AreaCoordinatorContact.MEMBER_ORGANIZATION_TYPE_OF_CHOICES,
    )
    ...
    street_type = serializers.ChoiceField(choices=AreaCoordinatorAddress.STREET_TYPE_CHOICES,)
    street_type_other = serializers.CharField(required=False, allow_null=True, allow_blank=True,)
    street_direction = serializers.ChoiceField(choices=AreaCoordinatorAddress.STREET_DIRECTION_CHOICES,)
    ...
    def validate_organization_name(self, value):
        ...
        of area_coordinator but then forgetting to fill in the `organization_name` field.
        """
        request = self.context.get('request')
        type_of = request.data.get('type_of')
        if type_of == AreaCoordinator.MEMBER_TYPE_OF.BUSINESS:
    ...
    def create(self, validated_data):
        ...
        # Attach the user to the `AreaCoordinator` group.
        user.groups.add(SharedGroup.GROUP_MEMBERSHIP.MEMBER)
        logger.info("Shared user assigned as area_coordinator.")

        area_coordinator = AreaCoordinator.objects.create(
            user=user,
            type_of=type_of,
        ...
        logger.info("Created area_coordinator.")
        ...
        if type_of != AreaCoordinator.MEMBER_TYPE_OF.BUSINESS:
            ...
            organization_type_of = AreaCoordinatorContact.MEMBER_ORGANIZATION_TYPE_OF.UNSPECIFIED

        area_coordinator_contact = AreaCoordinatorContact.objects.create(
        ...
        logger.info("Created area_coordinator contact.")
        ...
        street_direction = validated_data.get('street_direction', AreaCoordinatorAddress.STREET_DIRECTION.NONE)
        ...
        logger.info("Created area_coordinator address.")
        ...
        area_coordinator_metric = AreaCoordinatorMetric.objects.create(
            area_coordinator=area_coordinator,
            ...
            another_household_area_coordinator_registered=another_household_area_coordinator_registered,
        ...
        logger.info("Created area_coordinator metric.")

        # Attached our tags.
        tags = validated_data.get('tags', None)
        if tags is not None:
            if len(tags) > 0:
                area_coordinator_metric.tags.set(tags)
