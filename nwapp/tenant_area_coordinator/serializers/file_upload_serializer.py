...
class AreaCoordinatorFileUploadListCreateSerializer(serializers.ModelSerializer):
    area_coordinator = serializers.PrimaryKeyRelatedField(many=False, queryset=AreaCoordinator.objects.all(), required=True,)
    ...
    def create(self, validated_data):
        ...
        """
        area_coordinator = validated_data.get('area_coordinator')
        ...
        # Attach our comment.
        #---------------------------
        text = _("A file named \"%(filename)s\" has been uploaded to this area_coordinator's record by %(name)s.") % {
