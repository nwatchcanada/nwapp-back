...
from tenant_foundation.models import AreaCoordinator, AreaCoordinatorAddress


class Command(BaseCommand):
    help = _('Command will attempt to lookup the longitude and latitude of the area coordinator\'s address from a third-party geocoding API web-service.')

    def add_arguments(self, parser):
        """
        Run manually in console:
        python manage.py geocode_area_coordinator_address "london" "anthony-aguirre"
        """
        parser.add_argument('schema_name', nargs='+', type=str)
        parser.add_argument('area_coordinator_slug', nargs='+', type=str)
    ...
    def process(self, area_coordinator_address):
        ...
        if location:
            ...
            self.stdout.write(
                self.style.SUCCESS(_('Succeeded geocoding area coordinator %(slug)s with latitude and longitude of %(lt)s, %(ln)s')%{
            ...
            self.stdout.write(
                self.style.WARNING(_('Failed geocoding area coordinator %(slug)s')%{
                    'slug': area_coordinator_address.member.user.slug,
                })
            )
    ...
    def handle(self, *args, **options):
        ...
        area_coordinator_slug = options['area_coordinator_slug'][0]
        ...
        # Lookup our area_coordinator.
        area_coordinator_address = AreaCoordinatorAddress.objects.filter(
            member__user__slug=area_coordinator_slug
        ).first()
        if area_coordinator_address is None:
            raise CommandError(_('Area coordinator\'s address does not exist!'))

        if area_coordinator_address.needs_geocoding is False:
            raise CommandError(_('Area coordinator\'s geo-location was already found.'))
