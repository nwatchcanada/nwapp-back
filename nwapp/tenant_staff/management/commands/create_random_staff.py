...
class Command(BaseCommand):
    help = _('Command will generate random staves.')

    def add_arguments(self, parser):
        """
        Run manually in console:
        python manage.py create_random_staff "london" 1
        """
    ...
    def handle(self, *args, **options):
        ...
        for member in members:
            ...
            faker = Faker('en_CA')
            role_id = faker.pyint(min_value=2, max_value=3, step=1)

            # Promote the `member` to be an `staff`.
            staff = member.promote_to_staff(defaults={
                'role_id': role_id,
            ...
            self.stdout.write(
                self.style.SUCCESS(_('Promoted member slug %(slug)s to staff.')%{
