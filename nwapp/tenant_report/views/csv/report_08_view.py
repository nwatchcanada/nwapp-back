...
def report_08_streaming_csv_view(request):
    today = timezone.now()
    watches = Watch.objects.filter(Q(governors=None)).order_by("id")
    ...
    rows = (["Unassignment Watch Report","","","",],)
    ...
    response['Content-Disposition'] = 'attachment; filename="unassigned_watches.csv"'
    return response
