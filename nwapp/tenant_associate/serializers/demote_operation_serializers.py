...
class AssociateDemoteOperationSerializer(serializers.Serializer):
    ...
    reason = serializers.ChoiceField(
        write_only=True,
        required=True,
        choices=Associate.DEMOTION_REASON_CHOICES
    )
    ...
    def validate(self, dirty_data):
        ...
        if associate.user.role_id != SharedGroup.GROUP_MEMBERSHIP.ASSOCIATE and associate.user.role_id != SharedGroup.GROUP_MEMBERSHIP.FRONTLINE_STAFF:
            raise serializers.ValidationError(_("You cannot demote because this user is not associate!"))
        return dirty_data
    ...
    def create(self, validated_data):
        ...
        associate.demotion_reason = reason
        associate.demotion_reason_other = reason_other
        ...
        associate.user.groups.add(role_id)

        #---------------------------
        # Attach our comment.
        #---------------------------
        text = _("Demoted associate by %(name)s.") % {
